...
const ChatComp = ({ chat, index, length }) => {
  ...
  return (
    <div
      className={`${chat.role === "assistant" && "dark:bg-white/5 bg-black/5"}`}
      key={index}
    >
      <div className={`flex gap-4 p-4 sm:px-2 md:px-3 max-w-3xl mx-auto`}>
        {chat.role === "user" ? (
        ...
        <div className="pt-1">
          <span
            className={`dark:text-white/80 text-black/80 text-sm md:text-base ${
              chat.content && "mr-1"
            }`}
          >
            {chat.content}
          ...
          {chat.role === "assistant" &&
            length === index + 1 &&
            !doneStreaming && (
              <span className="border-r-8 dark:border-white/80 border-black/80 animate-pulse"></span>
            )}
        </div>
      </div>
    </div>
